...
- PDKT memisahkan mailbox read path menjadi list ringan dan detail lengkap. List memilih kolom scalar saja dan mengembalikan `PdktMailboxListItem`; empat kolom snapshot JSON yang dapat membawa base64 tidak diambil. Detail per-id mengembalikan `PdktMailboxItem` lengkap secara lazy setelah item dipilih. Browser tidak membaca tabel secara langsung: kedua read tetap melewati Hono API memakai user JWT agar RLS berlaku.
- Module settings (KETIK, PDKT, Telefun) disimpan namespaced di `user_settings.settings.<module>` agar tidak saling timpa. Setiap modul wajib membaca existing settings sebelum menulis. Landing dan workspace PDKT meneruskan serta menyinkronkan revision `x-settings-version` agar reuse data awal tidak melemahkan optimistic concurrency.
