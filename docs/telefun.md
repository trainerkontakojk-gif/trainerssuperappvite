...
- Phase 7 Full candidate memisahkan response generation dari `output_audio_buffer.started`/`stopped` dan state HTML media. Barge-in hanya menargetkan response audible yang sudah mempunyai kemajuan playback; blocked autoplay, pause/stall/end, serta interval hold/muted-output tidak dapat memicu interruption.
- Interruption WebRTC memakai scoped `response.cancel(response_id)` hanya ketika response masih in progress, lalu `output_audio_buffer.clear`. Jika cancel gagal dikirim sinkron, clear tidak diteruskan dan speech event berikutnya boleh mencoba urutan itu lagi; audio response yang sudah terminal boleh langsung di-clear. Clear buffer WebRTC adalah authority truncation conversation; browser tidak mengirim `conversation.item.truncate` kedua dengan estimasi `audio_end_ms`. Command cancel/clear dideduplikasi; exact cancel race yang berkorelasi melalui `event_id` tidak mengakhiri call, sedangkan provider error lain tetap fail-closed.
...
- Arbiter single-owner meminta response untuk user turn hanya setelah `input_audio_buffer.committed` dengan `item_id` unik. Time cue memakai kontrak existing `[TELEFUN_CONTROL:TIME_CUE]` sebagai system item; bila cue berimpit dengan turn VAD, response aktif, atau acknowledgement create, event create lengkap ditunda dan pending terakhir dicoalesce sambil mempertahankan `event_id`. Commit VAD memakai pending cue yang sama agar satu response melihat seluruh conversation item yang sudah committed. Setiap outbound create mendapat marker internal bounded di `response.metadata`; hanya `response.created` dengan marker exact lalu terminal response yang sama yang melepaskan acknowledgement. Response dengan metadata absent/null, shape lain, atau marker mismatch tetap dianggap lifecycle aktif tetapi origin-nya unknown dan tidak dapat melepaskan barrier Telefun. Stale/duplicate terminal tidak memicu create tambahan; shutdown membuang pending/barrier sinkron. Browser tetap tidak memperoleh `session.update` atau authority model/voice/instructions.
...
- Dua acceptance production pada kontrak dual-owner sebelumnya gagal dengan `conversation_already_has_active_response`. Acceptance ketiga pada candidate single-owner tidak mengulang collision itu dan menyelesaikan tiga response, tetapi gagal sesudah dua barge-in dengan provider code `invalid_value`. Bukti struktural dan kontrak OpenAI WebRTC menunjuk explicit item truncate setelah buffer clear sebagai kandidat terkuat; repair menghapus writer tersebut dan menambahkan observability `param` berbasis allowlist bounded tanpa raw message. Watchdog setiap window menutup flags, menghentikan Telefun, dan menguras durable boundary tanpa mengubah Gemini. Source repair sudah committed/pushed dan mempunyai deployment status, tetapi live production PASS sesudah repair terbaru belum diklaim. Mini, fallback policy, parity lintas browser, serta keputusan deprecation OpenAI WebSocket tetap deferred; Gemini dan legacy OpenAI WebSocket tidak berubah.
- Penyimpanan settings Telefun bersifat Promise-owned dan single-flight. Modal tetap terbuka serta seluruh aksi close/cancel/save dinonaktifkan sampai persistence selesai; kegagalan mempertahankan draft untuk retry. Start Call karena itu tidak dapat membaca model/transport lama sesudah user menekan Simpan.
- Worker WebRTC menyimpan `score` dan `voice_assessment` melalui RPC service-role. Karena lifecycle WebRTC melarang browser menulis field scoring, endpoint history list/detail membangun Feedback deterministik untuk sesi `openai-webrtc` berstatus `completed` dari assessment tervalidasi ketika kolom legacy `feedback` kosong; feedback yang sudah dipersist tetap dipertahankan.
