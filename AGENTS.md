...
- `apps/api/src/services/sidak/dashboard-forecast.ts` — **Phase 200**: Added `buildParameterSummary()`, `formatParameterBlock()`, `formatFindingDelta()`, `normalizeForecastInsightText()` — deterministic three-block parameter normalization
- `apps/web/src/components/sidak/forecast-insight-parser.ts` — **Phase 200**: Strip trailing colon from subsection title in `parseSubsections`
- `apps/api/src/__tests__/sidak-dashboard-forecast.test.ts` — **Phase 200**: Tests for parameter block normalization, three-block layout
- `apps/web/src/__tests__/forecast-insight-panel.test.tsx` — **Phase 200**: Tests for "Stabil" subsection with neutral tone, 3 subsections
- `docs/SIDAK_LOGIC_AND_SCORING.md` — **Phase 200**: Added note about backend parameter normalization fallback
- `docs/rebuild-logs/phase-200-sidak-forecast-parameter-normalization.md` — **NEW Phase 200**: Documentation for forecast parameter normalization & three-block layout
- `apps/web/src/lib/scoring.ts` — **Phase 201**: Added `DEFAULT_SERVICE_FOLDER_MAP` constant mapping service types to default folder names (call→Tim Call, chat→Tim Whatsapp, email→Tim Email, cso→Tim Mix, pencatatan→Tim Mix, bko→Tim BKO)
- `apps/web/src/routes/sidak/dashboard.tsx` — **Phase 201**: Folder state persistence via `allFolders`+`setAllFolders`; `maxVisibleParameters=2`; total trend toggle; default folder pairing on service change
- `apps/web/src/routes/sidak/ranking.tsx` — **Phase 201**: Folder state persistence; `initialFolderSetRef` one-time pairing; auto-select folder from `DEFAULT_SERVICE_FOLDER_MAP` on initial load and service change
- `apps/web/src/components/sidak/ParamTrendChart.tsx` — **Phase 201**: Multi-forecast series support via `forecastResults` array prop; normalized via `useMemo`
- `apps/web/src/__tests__/sidak-filter-pairing.test.tsx` — **NEW Phase 201**: Dashboard + Ranking filter pairing regression tests (folder persistence, auto-pair on service change)
