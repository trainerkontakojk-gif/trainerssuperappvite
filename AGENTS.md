...
- **Audio recordings are automatically remuxed with FFmpeg after upload to make them seekable (play/pause/seek). FFmpeg must be installed in the deployment container via `RAILPACK_DEPLOY_APT_PACKAGES=ffmpeg`. The remux is fire-and-forget (non-blocking) and best-effort — if FFmpeg is unavailable, original recordings are preserved as-is.**
