...
vi.mock("../store/authStore", () => ({
  useAuthStore: (selector: (s: any) => any) =>
    selector({
      ...
      session: null,
      setSession: mockSetState,
      setProfile: mockSetState,
    }),
}));
...
const SAMPLE_HISTORY = [
  ...
  },
  {
    ...
    user_id: "u3",
    module: "telefun",
    scenario_title: "Skenario C",
    created_at: "2025-05-03T09:00:00Z",
    duration_seconds: 300,
    score: 45,
    history: "http://rec.url",
...
const SAMPLE_PRICING = [
  {
    ...
    provider: "gemini",
    input_price_usd_per_million: 0.15,
    output_price_usd_per_million: 0.6,
  },
];
...
describe(
  "MonitoringPage - Unauthorized & Visual Parity Fix",
  { timeout: 30000 },
  () => {
    beforeEach(() => {
      vi.clearAllMocks();
      ...
      mockAiClient["monitoring/aggregation"].$get.mockResolvedValue(
        SAMPLE_AGGREGATION,
      );
    ...
    describe("Auth Transport", () => {
      it("calls aiClient monitoring/history $get", async () => {
        ...
        await waitFor(() =>
          expect(mockAiClient["monitoring/history"].$get).toHaveBeenCalled(),
        );
      }, 30000);
      ...
      it("does NOT call global fetch (raw unauthenticated) for monitoring endpoints", async () => {
        ...
        const monitoringCalls = rawFetch.mock.calls.filter(
          ([input]: any[]) =>
            typeof input === "string" && input.includes("/ai/monitoring"),
        );
    ...
    describe("Error Mapping", () => {
      it("maps Unauthorized to user-friendly message", async () => {
        mockAiClient["monitoring/history"].$get.mockRejectedValue(
    ...
    describe("Feature Regression", () => {
      ...
      it("switches to usage tab and fetches aggregation", async () => {
        ...
        await waitFor(() =>
          expect(
            mockAiClient["monitoring/aggregation"].$get,
          ).toHaveBeenCalled(),
        );
      });
