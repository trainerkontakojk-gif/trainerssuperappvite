...
describe('PDKT Landing Page', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockUseApi.mockReturnValue({ data: null, loading: false, refetch: vi.fn() });
    mockGetApi.mockImplementation((url: string) => {
      if (url === '/pdkt/settings') return Promise.resolve({ success: true, settings: null });
      if (url === '/pdkt/history') return Promise.resolve([]);
      if (url.startsWith('/ai/usage/summary')) return Promise.resolve({ totalCalls: 0, totalTokens: 0, totalCostIdr: 0 });
  ...
  it('renders correct heading', async () => {
    render(<PdktLanding />);
    await screen.findByText('Buka simulasi email dengan pengalaman workspace yang seragam.');
  });
  ...
  it('renders action buttons in exact order', async () => {
    ...
    expect(docPosition(mulaiBtn.closest('button')!)).toBeLessThan(docPosition(pengaturanBtn.closest('button')!));
    expect(docPosition(pengaturanBtn.closest('button')!)).toBeLessThan(docPosition(riwayatBtn.closest('button')!));
    expect(docPosition(riwayatBtn.closest('button')!)).toBeLessThan(docPosition(usageBtn.closest('button')!));
  });
  ...
  it('does not render old card grid copy', async () => {
    ...
    expect(screen.queryByText('Pilih skenario dan mulai simulasi email')).toBeNull();
  });

  it('opens mailbox workspace when Mulai Simulasi is clicked', async () => {
    mockUseApi.mockImplementation((url: string) => {
      if (url === '/pdkt/mailbox') return { data: [], loading: false, refetch: vi.fn() };
      if (url === '/pdkt/scenarios') return { data: [], loading: false, refetch: vi.fn() };
      if (url === '/pdkt/consumer-types') return { data: [], loading: false, refetch: vi.fn() };
