...
vi.mock('../lib/fetchAuthProfile', () => ({
  fetchAuthProfile: vi.fn().mockResolvedValue({ id: 'u1', email: 'test@x.com', role: 'agent', status: 'active' }),
}));
...
describe('initAuth', () => {
  ...
  it('restores session from stored token', async () => {
    ...
    mockGetSession.mockResolvedValue({ data: { session: { access_token: 'tok1', user: { id: 'u1' } } }, error: null });

    await initAuth();

    expect(mockGetUser).toHaveBeenCalledWith('tok1');
    expect(useAuthStore.getState().session).toEqual({ access_token: 'tok1', user: { id: 'u1' } });
  });
