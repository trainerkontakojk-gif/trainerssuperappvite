...
describe('PDKT Settings Helpers', () => {
  ...
  it('coerces invalid model to default', () => {
    ...
    expect(coercePdktModelId('gemini-3.1-pro-preview')).toBe('gemini-3.1-pro-preview');
  });
  ...
  it('overrides identity with custom identity', () => {
    ...
    const config = generatePdktSessionConfig(settingsWithCustomIdentity, mockScenario, mockFallbackIdentity);
  ...
  it('uses fallback identity when no custom identity is provided', () => {
    const config = generatePdktSessionConfig(mockSettings, mockScenario, mockFallbackIdentity);
