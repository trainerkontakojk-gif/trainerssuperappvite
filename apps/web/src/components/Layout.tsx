...
const SIDAK_CHILDREN = [
  ...
  { to: '/sidak/reports', label: 'Laporan', allowedRoles: ['trainer', 'admin'] },
  { to: '/sidak/input', label: 'Input Temuan', allowedRoles: ['trainer', 'admin'] },
  { to: '/sidak/periods', label: 'Periode QA', allowedRoles: ['trainer', 'admin'] },
  { to: '/sidak/settings', label: 'Parameter QA', allowedRoles: ['trainer', 'admin'] },
];

const MANAGEMENT_LINKS = [
  { to: '/dashboard/users', label: 'User Management', icon: Shield, allowedRoles: ['trainer', 'admin'] },
  { to: '/dashboard/access-approval', label: 'Access Approval', icon: UserCheck, allowedRoles: ['trainer', 'admin'] },
  { to: '/dashboard/access-groups', label: 'Access Groups', icon: Layers, allowedRoles: ['trainer', 'admin'] },
  { to: '/dashboard/monitoring', label: 'Monitoring', icon: Activity, allowedRoles: ['trainer', 'leader', 'admin'] },
  { to: '/dashboard/activities', label: 'Activity Logs', icon: History, allowedRoles: ['trainer', 'admin'] },
];
...
function getHeaderContent(pathname: string) {
  ...
  if (pathname.startsWith('/sidak')) {
    return { eyebrow: 'Sistem Informasi Data Analisis Kualitas', title: 'SIDAK' };
  }
  ...
  if (pathname.startsWith('/monitoring') || pathname === '/dashboard/monitoring') {
...
export function DashboardLayout() {
  ...
  const isPublicRoute = ['/', '/waiting-approval', '/reset-password'].includes(pathname);
  ...
  const visibleModules = APP_MODULES.filter((module) =>
    ['dashboard', 'ketik', 'pdkt', 'telefun', 'profiler'].includes(module.id) && isRoleAllowed(profile?.role, module.allowedRoles)
  );

  const qaModule = APP_MODULES.find((module) => module.id === 'qa-analyzer');
  const isQaAllowed = qaModule && isRoleAllowed(profile?.role, qaModule.allowedRoles);
    ...
    <div className="flex h-screen bg-background text-foreground transition-colors duration-300">
      ...
      >
        <div className="flex flex-1 flex-col overflow-hidden p-6">
          {/* Brand block */}
          <div className={`mb-8 flex items-center overflow-hidden ${effectiveIsCollapsed ? 'lg:justify-center' : 'justify-between'}`}>
            <Link to="/dashboard" className="flex items-center gap-3">
              ...
              {!effectiveIsCollapsed && (
                <div className="flex flex-col">
                  <span className="font-display text-sm font-semibold tracking-tight">Trainers SuperApp</span>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  {!effectiveIsCollapsed && (
                    <>
                      ...
                      <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${sidakOpen ? 'rotate-180' : ''}`} />
                    </>
                  )}
                </button>
                <div
                ...
                >
                  <div className="space-y-1 px-2 pb-2 pl-11 pt-2">
                    {SIDAK_CHILDREN.filter((item) => isRoleAllowed(profile?.role, item.allowedRoles)).map((item) => {
          ...
          <div className="mt-auto space-y-2 border-t border-border/40 pt-6">
            {!effectiveIsCollapsed && (
              <div className="rounded-3xl border border-border/50 bg-background/60 px-4 py-4">
                ...
                <p className="mt-2 truncate text-sm font-semibold text-foreground" title={profile?.email || session?.user?.email}>
                  {profile?.email || session?.user?.email}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">Role: {normalizeRoleLabel(profile?.role)}</p>
              </div>
            )}
