...
export function resolveConsumerNameMentionPattern(
  ...
  const patterns: ResolvedConsumerNameMentionPattern[] = ['upfront', 'middle', 'late', 'none'];
...
export function generatePdktSessionConfig(
  ...
  const identity: PdktIdentity = {
    ...
    bodyName: customIdentity?.bodyName || (customIdentity?.senderName || fallbackIdentity.bodyName),
  };
