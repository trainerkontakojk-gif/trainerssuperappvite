...
import { Mail, Search, Inbox, Send, Trash2, Plus, Settings, History, BarChart3 } from 'lucide-react';
  ...
  return (
    <div className="w-full md:w-80 border-r border-gray-200 flex flex-col h-full bg-white shrink-0">
            ...
            {filteredItems.map((item) => (
              ...
              >
                <div className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                  selectedId === item.id ? 'bg-sky-600 text-white shadow-md' : 'bg-gray-100 text-gray-600'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className={`text-xs truncate ${item.status === 'open' ? 'font-bold text-gray-900' : 'font-medium text-gray-600'}`}>
                      {item.sender_name}
                    </span>
                  ...
                  <div className={`text-[11px] truncate mb-0.5 ${item.status === 'open' ? 'font-bold text-gray-900' : 'font-medium text-gray-600'}`}>
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    {item.status === 'open' ? (
                      <span className="inline-flex items-center gap-1 font-medium text-[9px] text-sky-600">
                        <span className="w-1.5 h-1.5 rounded-full bg-sky-600" /> Menunggu Balasan
                      </span>
                    ) : (
      ...
      <div className="p-3 border-t border-gray-200 bg-gray-50 flex items-center justify-around">
        <button className="p-2 text-sky-600 hover:bg-sky-100 rounded-xl transition-all" title="Inbox">
          <Inbox className="w-4 h-4" />
        </button>
        <button className="p-2 text-gray-400 hover:bg-gray-100 rounded-xl transition-all" title="Sent (History)">
          <Send className="w-4 h-4" />
        </button>
        <button className="p-2 text-gray-400 hover:bg-gray-100 rounded-xl transition-all" title="Trash">
