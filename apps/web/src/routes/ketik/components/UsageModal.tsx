...
interface UsageModalProps {
  ...
  sessionDelta?: { costIdr: number; totalTokens: number; totalCalls: number } | null;
...
export function UsageModal({ isOpen, onClose, module = 'ketik', sessionDelta, sessionDeltaPending }: UsageModalProps) {
        ...
        <header className="px-5 py-4 sm:px-6 sm:py-5 border-b flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground font-medium">Modul {moduleLabel}</p>
            </div>
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-foreground/[0.02] rounded-xl p-4">
                  ...
                  <div className="text-base font-bold text-muted-foreground">{formatTokenCount(usage.total_input_tokens)}</div>
                </div>
                <div className="bg-foreground/[0.02] rounded-xl p-4">
                  <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Output Tokens</div>
                  <div className="text-base font-bold text-muted-foreground">{formatTokenCount(usage.total_output_tokens)}</div>
                </div>
              </div>
            </>
          ) : (
