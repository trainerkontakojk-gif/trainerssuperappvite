  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      <header className="mb-8">
        ...
        <h1 className="mt-2 text-2xl font-bold text-gray-900">Pengaturan profil</h1>
        <p className="mt-2 text-sm text-gray-500">Kelola nama tampilan dan password akun Anda dari satu halaman.</p>
      </header>

      {error && <p className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}

      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        <h2 className="text-base font-semibold text-gray-900">Profil pengguna</h2>
        <p className="mt-1 text-xs text-gray-500">Email login: {loadingProfile ? 'Memuat...' : email || '-'}</p>
        <form onSubmit={handleSaveName} className="mt-5 space-y-4">
          <label className="block">
            <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-500">Nama tampil</span>
            <input
              type="text"
              value={fullName}
      ...
      <section className="mt-6 rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-xs text-gray-500">Gunakan minimal 6 karakter agar akun tetap aman.</p>
        <form onSubmit={handleSavePassword} className="mt-5 space-y-4">
          <label className="block">
            <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-500">Password baru</span>
            <input
              type="password"
              value={newPassword}
          ...
          <label className="block">
            <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-500">Konfirmasi password</span>
            <input
              type="password"
