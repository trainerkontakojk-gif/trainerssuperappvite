...
export default function AccessGroupsPage() {
  const { data: groups, loading: loadingGroups, refetch: refetchGroups } = useApi<AccessGroup[]>('/admin/access-groups');
  const { data: scopeOptions } = useApi<ScopeOptions>('/admin/access-scope-options');

  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(null);
  const { data: selectedGroupItems, loading: loadingItems, refetch: refetchItems } = useApi<GroupItem[]>(
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-bold tracking-tight text-gray-900">Grup Akses</h2>
          <p className="mt-1 text-gray-500">
            Definisikan batasan wilayah kerja (skup data) untuk Leader berdasarkan kriteria dinamis.
          </p>
        </div>
        <button
          ...
          <div className="rounded-2xl border bg-white p-2 shadow-sm space-y-1 max-h-[600px] overflow-y-auto">
            {loadingGroups ? (
              <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                ...
                <span className="mt-2 text-xs font-semibold">Memuat grup akses...</span>
              </div>
            ) : filteredGroups.length === 0 ? (
              <div className="py-8 text-center text-gray-400">
                <Layers className="mx-auto h-8 w-8 text-gray-300 mb-2" />
                <p className="text-xs font-semibold">Tidak ada grup ditemukan</p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${
                      g.is_active !== false ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-600'
                    }`}>
          ...
          {selectedGroup ? (
            <div className="rounded-2xl border bg-white p-6 shadow-sm space-y-6">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 border-b pb-6">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">{selectedGroup.description || 'Tidak ada deskripsi.'}</p>
                </div>
                <button
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {loadingItems ? (
                    <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                      <div className="h-6 w-6 animate-spin rounded-full border-2 border-indigo-600 border-t-transparent" />
                      <span className="mt-2 text-xs font-medium">Memuat aturan akses...</span>
                    </div>
                    ...
                    <div className="rounded-xl border border-dashed py-10 text-center bg-gray-50/50">
                      ...
                      <p className="text-xs font-semibold text-gray-600">Belum ada aturan akses</p>
                      <p className="text-[10px] text-gray-400 mt-0.5">Semua data peserta terproteksi penuh hingga aturan dibuat.</p>
                    </div>
                  ) : (
              ...
              {/* Add Rule Form */}
              {scopeOptions && (
                <form onSubmit={handleAddRule} className="rounded-xl border bg-indigo-50/30 p-5 space-y-4">
                  <div>
                    <h5 className="text-xs font-bold text-indigo-950 uppercase tracking-wider">Tambah Aturan Baru</h5>
                    <p className="text-[11px] text-indigo-700 mt-0.5">Pilih kriteria kolom data beserta nilainya.</p>
                  </div>

                  <div className="grid gap-4 sm:grid-cols-[1fr_1fr_auto]">
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Kolom</label>
                      <select
                        value={newFieldName}
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Nilai Parameter</label>
                      <select
                        value={newFieldValue}
            ...
            <div className="rounded-2xl border border-dashed py-24 text-center bg-white shadow-sm flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-gray-500 mt-1 max-w-sm">
                Pilih salah satu grup akses di sebelah kiri untuk melihat detail atau mengelola aturan wilayah kerja.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border bg-white p-6 shadow-xl animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-xs text-gray-500 mb-4">
              Nama grup sebaiknya representatif untuk mempermudah identifikasi penugasan leader.
            </p>

            <form onSubmit={handleSaveGroup} className="space-y-4">
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Nama Grup</label>
                <input
                  type="text"
                  required
