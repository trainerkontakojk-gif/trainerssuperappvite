...
export default function UsersPage() {
  ...
  const { data: initialUsers, loading, refetch } = useApi<ManagedUser[]>('/admin/users');
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-bold tracking-tight text-gray-900">Kelola Pengguna</h2>
          <p className="mt-1 text-gray-500">
            Approval, suspend, role access, dan reset password dalam satu panel aksi terpusat.
          </p>
        </div>
        <div className="rounded-2xl border bg-white p-4 shadow-sm min-w-[240px]">
          <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Operator Aktif</p>
          <p className="mt-1 text-sm font-semibold text-gray-900">{currentProfile?.email || 'System User'}</p>
          <p className="text-xs text-indigo-600 font-medium">Hak kelola: {normalizeRoleLabel(managerRole)}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
        ...
        <div className="flex items-center gap-1.5 rounded-xl border bg-white p-1 shadow-sm">
          {[
            ...
            >
              <span>{tab.label}</span>
              <span className={`rounded px-1.5 py-0.5 text-[10px] ${activeTab === tab.id ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                {tab.count}
              </span>
            </button>
          ))}
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <span className="mt-4 text-sm font-medium">Memproses data pengguna...</span>
          </div>
        ) : filteredUsers.length === 0 ? (
          <div className="rounded-2xl border border-dashed py-20 text-center bg-white shadow-sm">
            <Users className="mx-auto h-12 w-12 text-gray-300" />
            <p className="mt-4 font-semibold text-gray-900">Tidak ada pengguna ditemukan</p>
            ...
            return (
              ...
              >
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm text-gray-500 break-all">{entry.email}</p>
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-400">
                        <span>ID: {entry.id.slice(0, 8)}...</span>
                        <span>•</span>
                        <span>Daftar: {new Date(entry.created_at).toLocaleDateString('id-ID')}</span>
                        <span>•</span>
                        <span>Role: <strong className="text-gray-600">{normalizeRoleLabel(entry.role)}</strong></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-xl border p-4 bg-gray-50/50">
                      ...
                      <p className="mt-2 text-xs text-gray-500 leading-relaxed">
                        Kirim link reset password langsung ke email terdaftar pengguna.
                      </p>
                    </div>
                    <div className="rounded-xl border p-4 bg-gray-50/50">
                      <div className="flex items-center gap-2 text-xs font-bold text-gray-700 uppercase tracking-wide">
                      ...
                      <p className="mt-2 text-xs text-gray-500 leading-relaxed">
                        Atur wewenang role, status aktif, atau hapus permanen akun.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl border bg-gray-50/50 p-5 flex flex-col justify-between">
                  <div className="space-y-4">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Pengaturan Akses</p>
                      <h4 className="mt-1 text-sm font-bold text-gray-800">Ubah Role & lifecycle</h4>
                    </div>

                    <div className="space-y-3">
                      <div>
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Role Akses</label>
                        <div className="flex gap-2">
                          <select
                            value={selectedRoles[entry.id] || normalizedEntryRole}
                            onChange={(e) => {
                              const nextRole = e.target.value;
                              setSelectedRoles((prev) => ({ ...prev, [entry.id]: nextRole }));
                            }}
