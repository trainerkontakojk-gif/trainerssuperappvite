...
export default function ProfilerTable() {
  ...
  const sortedFiltered = [...filtered].sort((a, b) => {
    ...
    return (
      (a.nomor_urut || 0) - (b.nomor_urut || 0) || a.nama.localeCompare(b.nama)
    );
  });
  ...
  const saveBulkOrder = async () => {
    ...
    try {
      ...
      setPeserta((prev) =>
        prev.map((p) => ({
          ...p,
          nomor_urut: reorderMap[p.id] || p.nomor_urut,
        })),
      );
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="text-right p-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    Aksi
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {paginatedData.map((p, idx) => (
                  <tr
                    key={p.id}
                    className={`hover:bg-gray-50 transition-colors ${editingId === p.id ? "bg-amber-50" : ""} ${isReorderMode ? "cursor-move" : ""}`}
                  >
                    <td className="p-3">
                      {isReorderMode ? (
                        <input
                        ...
                        <span className="text-xs font-bold text-gray-400">
                          {p.nomor_urut || 0 || (page - 1) * pageSize + idx + 1}
                        </span>
                      )}
                    </td>
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      {editingId === p.id ? (
                        <input
                          value={editForm.tim ?? ""}
                          onChange={(e) =>
                            setEditForm((f) => ({ ...f, tim: e.target.value }))
                          }
                          className="w-full rounded border px-2 py-1 text-sm outline-none focus:border-indigo-500"
                        />
                      ) : (
