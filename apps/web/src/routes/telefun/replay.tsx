...
export default function TelefunReplay() {
  ...
  const { data: annotations, refetch: refetchAnnotations } = useApi<TelefunReplayAnnotation[]>(`/telefun/annotations/${id}`);

  const [newAnnotation, setNewAnnotation] = useState({ text: '', category: 'strength' as any });
  ...
  if (!session) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/telefun" className="text-indigo-600 hover:underline mt-4 inline-block">Kembali</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between">
        <Link to="/telefun" className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl border shadow-sm p-8 space-y-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <button onClick={() => handleSeek(0)} className="p-2 text-gray-400 hover:text-gray-600 transition-colors">
                  <RotateCcw size={20} />
                </button>
          ...
          <div className="bg-white rounded-2xl border shadow-sm p-6">
            ...
            <div className="grid gap-3">
              {summary?.recommendations && Array.isArray(summary.recommendations) ? (
                summary.recommendations.slice(0, 5).map((rec: string, i: number) => (
                ...
                <p className="text-sm text-gray-400 italic">Belum ada ringkasan coaching.</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border shadow-sm flex flex-col h-full max-h-[600px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {annotations?.map((anno) => (
                <div key={anno.id} className="group relative p-3 rounded-xl border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      {anno.category === 'strength' && <Star size={12} className="text-emerald-500 fill-emerald-500" />}
                      {anno.category === 'improvement_area' && <AlertTriangle size={12} className="text-amber-500 fill-amber-500" />}
                      {anno.category === 'critical_moment' && <Zap size={12} className="text-red-500 fill-red-500" />}
                      <span className="text-[9px] font-bold uppercase tracking-wide text-gray-400">
                        {anno.category.replace('_', ' ')}
                      </span>
                    </div>
                  </div>
                  <p className="text-xs text-gray-700 leading-relaxed">{anno.text}</p>
                  {anno.is_manual && (
                    <button 
            ...
            <div className="p-4 border-t border-gray-100 space-y-3">
              <div className="flex gap-2">
                {(['strength', 'improvement_area', 'critical_moment', 'technique_used'] as const).map(cat => (
                  <button 
                    key={cat}
                    onClick={() => setNewAnnotation(prev => ({ ...prev, category: cat }))}
                    className={`flex-1 py-1.5 rounded-lg text-[9px] font-bold uppercase transition-all ${
                      newAnnotation.category === cat ? 'bg-indigo-600 text-white shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                    }`}
                  >
                    {cat[0]}
                  </button>
              ...
              >
                {isAddingAnnotation ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}
                Add Annotation
              </button>
            </div>
          </div>
