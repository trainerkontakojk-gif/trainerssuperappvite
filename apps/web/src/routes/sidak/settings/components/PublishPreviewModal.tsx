  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-background/80 backdrop-blur-md" onClick={onClose}>
      ...
      >
        ...
        <h2 className="font-outfit text-xl font-bold text-foreground text-center">Preview & Publish</h2>
        ...
        <div className="space-y-3 border border-border rounded-xl p-4 bg-background/30">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-foreground">{SERVICE_LABELS[previewVersion.service_type]}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground font-medium">Version:</span>
            <span className="font-semibold text-foreground">v{getPreviewVersionNumber()}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground font-medium">Scoring Mode:</span>
            <span className="font-semibold text-foreground uppercase">{previewVersion.scoring_mode}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground font-medium">Total Parameter:</span>
            <span className="font-semibold text-foreground">{draftIndicators.length}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground font-medium">Critical Weight:</span>
            <span className="font-semibold text-foreground">{Math.round(previewVersion.critical_weight * 100)}%</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground font-medium">Non-Critical Weight:</span>
            <span className="font-semibold text-foreground">{Math.round(previewVersion.non_critical_weight * 100)}%</span>
          </div>
        </div>

        {/* Indicator List */}
        <div className="space-y-2">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground px-1">Parameter</p>
          <div className="max-h-40 overflow-y-auto space-y-1 border border-border rounded-xl p-3 bg-background/20">
            {draftIndicators.map((ind) => (
              <div key={ind.id} className="flex justify-between text-xs py-1.5 border-b border-border/50 last:border-0">
                <span className="font-medium text-foreground/85">{ind.name}</span>
        ...
        <label className="flex items-center gap-3 p-4 rounded-xl border border-border bg-transparent cursor-pointer">
          ...
          />
          <span className="text-xs font-semibold text-foreground">Saya telah meninjau parameter dan bobot di atas</span>
        </label>
