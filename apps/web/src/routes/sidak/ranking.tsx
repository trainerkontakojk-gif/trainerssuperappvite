...
export default function SidakRankingPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-500">Diurutkan berdasarkan jumlah defect (terbanyak ke tersedikit)</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-left">
                ...
                <th className="p-3 font-medium text-gray-500 text-right">Defects</th>
                <th className="p-3 font-medium text-gray-500 text-right">Score</th>
                <th className="p-3 font-medium text-gray-500 text-center">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {paginated.map((agent, i) => {
                ...
                <tr key={agent.agentId} className="hover:bg-gray-50">
                  ...
                  <td className={`p-3 text-right font-medium ${agent.defects > 0 ? 'text-red-600' : 'text-green-600'}`}>
                    {agent.defects}
                  </td>
                  <td className="p-3 text-right">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      agent.score >= 85 ? 'bg-green-100 text-green-700' :
                      agent.score >= 70 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'
                    }`}>
                  ...
                  <td className="p-3 text-center">
                    {agent.defects === 0 ? (
                      ...
                      <span className="text-amber-600 text-xs">Non-Critical</span>
                    )}
                  </td>
                </tr>
                );
