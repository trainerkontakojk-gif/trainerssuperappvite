...
export default function SidakInputPage() {
  ...
  const [items, setItems] = useState<Array<{ indicator_id: string; nilai: number; ketidaksesuaian: string; sebaiknya: string }>>([]);
  ...
  const addItem = () => {
    ...
    setItems(prev => [...prev, { indicator_id: filteredIndicators[0].id, nilai: 3, ketidaksesuaian: '', sebaiknya: '' }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await postApi('/sidak/temuan/batch', {
        peserta_id: selectedAgent, period_id: selectedPeriod, service_type: selectedService,
        no_tiket: noTiket || null,
        items: items.map(i => ({ indicator_id: i.indicator_id, nilai: i.nilai, ketidaksesuaian: i.ketidaksesuaian || null, sebaiknya: i.sebaiknya || null })),
      });
  ...
  const handleDownloadTemplate = async () => {
    ...
    a.href = url; a.download = `template-temuan-${selectedService}.xlsx`;
  ...
  const handleImport = async () => {
    ...
    try {
      await postApi('/sidak/temuan/batch', {
        peserta_id: selectedAgent, period_id: selectedPeriod, service_type: selectedService,
        items: valid.map(r => ({ indicator_id: r.indicator_id!, nilai: r.nilai, ketidaksesuaian: r.ketidaksesuaian || null, sebaiknya: r.sebaiknya || null })),
      });
      setMessage({ type: 'success', text: `${valid.length} temuan berhasil diimport!${invalid.length > 0 ? ` ${invalid.length} dilewati.` : ''}` });
      setImportRows([]); setShowImport(false);
    } catch (e: any) { setMessage({ type: 'error', text: e.message });
    } finally { setImporting(false); }
  };

  const { valid: validRows, invalid: invalidRows } = validateImportRows(importRows);
    ...
    <div className="space-y-6">
          ...
          {importRows.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1 text-sm text-green-600"><CheckCircle size={14} /> {validRows.length} Valid</span>
                <span className="flex items-center gap-1 text-sm text-red-600"><XCircle size={14} /> {invalidRows.length} Invalid</span>
              </div>
                  ...
                  <tbody>
                    {importRows.map((r, i) => (
                      <tr key={i} className={`border-t ${r.error ? 'bg-red-50' : ''}`}>
                        ...
                        <td className="p-2">
                          {r.error ? <span className="text-xs text-red-600">{r.error}</span> : <span className="text-xs text-green-600">OK</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <button onClick={handleImport} disabled={importing || validRows.length === 0}
                ...
                <Upload size={16} /> {importing ? 'Mengimport...' : `Import ${validRows.length} Temuan`}
              </button>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 bg-white rounded-xl border shadow-sm p-6">
        <div>
          ...
          <select className="w-full border rounded-lg p-2" value={selectedAgent} onChange={e => setSelectedAgent(e.target.value)}>
        ...
        <div>
          ...
          <select className="w-full border rounded-lg p-2" value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)}>
        ...
        <div>
          ...
          <select className="w-full border rounded-lg p-2" value={selectedService} onChange={e => { setSelectedService(e.target.value); setImportRows([]); }}>
            {['call', 'chat', 'email', 'cso', 'pencatatan', 'bko', 'slik'].map(s => (
        ...
        <div>
          ...
          <input className="w-full border rounded-lg p-2" value={noTiket} onChange={e => setNoTiket(e.target.value)} placeholder="Opsional" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border shadow-sm p-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={addItem} className="flex items-center gap-1 text-sm text-indigo-600 hover:text-indigo-700">
            <Plus size={16} /> Tambah Indikator
          </button>
        </div>

        {items.length === 0 ? (
          <p className="text-gray-400 text-sm py-4 text-center">Belum ada indikator. Klik "Tambah Indikator" untuk mulai.</p>
        ) : (
          <div className="space-y-2">
            {items.map((item, i) => {
              const ind = filteredIndicators.find(ind => ind.id === item.indicator_id);
              return (
                <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                  <select className="flex-1 border rounded-lg p-2 text-sm" value={item.indicator_id} onChange={e => updateItem(i, 'indicator_id', e.target.value)}>
                    {filteredIndicators.map(ind => (
                      <option key={ind.id} value={ind.id}>{ind.name} ({ind.category})</option>
                    ))}
                  </select>
                  <select className="w-20 border rounded-lg p-2 text-sm" value={item.nilai} onChange={e => updateItem(i, 'nilai', parseInt(e.target.value))}>
                    {[0, 1, 2, 3].map(n => <option key={n} value={n}>{n}</option>)}
                  </select>
                  <input className="flex-1 border rounded-lg p-2 text-sm" placeholder="Ketidaksesuaian" value={item.ketidaksesuaian} onChange={e => updateItem(i, 'ketidaksesuaian', e.target.value)} />
                  <input className="flex-1 border rounded-lg p-2 text-sm" placeholder="Sebaiknya" value={item.sebaiknya} onChange={e => updateItem(i, 'sebaiknya', e.target.value)} />
