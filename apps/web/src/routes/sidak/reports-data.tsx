                      ...
                      .map((r: any, i: number) => (
                        <tr key={r.id || i} className="hover:bg-gray-50">
                          ...
                          <td className="p-3 text-gray-600 font-mono text-xs">
                            {String(r.qa_periods?.month || "").padStart(2, "0")}
                            /{r.qa_periods?.year || ""}
                          </td>
                          <td className="p-3">
                            <span className="font-medium text-gray-900">
                              {r.profiler_peserta?.nama || "-"}
                            </span>
                            <span className="text-xs text-gray-400 ml-1">
                              ({r.profiler_peserta?.batch_name || ""})
                            </span>
                          </td>
                          <td className="p-3 text-gray-700">
                            {r.qa_indicators?.name || "-"}
                          </td>
                          <td className="p-3 text-rose-700 italic text-xs max-w-[200px] truncate">
                            {r.ketidaksesuaian || "-"}
                          </td>
                          <td className="p-3 text-emerald-700 text-xs max-w-[200px] truncate">
                            {r.sebaiknya || "-"}
                          </td>
