...
export default function SidakSettingsPage() {
  ...
  const [editingVersion, setEditingVersion] = useState<RuleVersion | null>(
    null,
  );
  const [updating, setUpdating] = useState(false);
  const [actionTarget, setActionTarget] = useState<{
    id: string;
    action: "publish" | "supersede";
  } | null>(null);
  ...
  } = useApi<ServiceWeightData[]>("/sidak/service-weights");
  const {
    data: versions,
    loading: versionsLoading,
    refetch: refetchVersions,
  } = useApi<RuleVersion[]>("/sidak/rule-versions");
  ...
  const [versionIndicatorsLoading, setVersionIndicatorsLoading] =
    useState(false);
  ...
  const [compareIndicatorsLoading, setCompareIndicatorsLoading] =
    useState(false);
  const { data: allIndicators } = useApi<any[]>("/sidak/indicators");
  const [addIndicatorVersionId, setAddIndicatorVersionId] = useState<
    string | null
  >(null);
  ...
  const handleConfirmAction = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (actionTarget.action === "publish") {
        await postApi(`/sidak/rule-versions/${actionTarget.id}/publish`, {
          change_reason,
        });
        notify.success("Versi published");
      } else {
        await postApi(`/sidak/rule-versions/${actionTarget.id}/supersede`, {
          change_reason,
        });
  ...
  const handleCreateDraft = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await postApi("/sidak/rule-versions", {
        ...
        critical_weight:
          parseFloat(form.get("critical_weight") as string) || 0.5,
        non_critical_weight:
          parseFloat(form.get("non_critical_weight") as string) || 0.5,
  ...
  const handleUpdateDraft = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await putApi(`/sidak/rule-versions/${editingVersion.id}`, {
        critical_weight:
          parseFloat(form.get("critical_weight") as string) ||
          editingVersion.critical_weight,
        non_critical_weight:
          parseFloat(form.get("non_critical_weight") as string) ||
          editingVersion.non_critical_weight,
  ...
  const fetchVersionIndicators = useCallback(async (versionId: string) => {
    ...
    try {
      const res = await getApi<{ success: boolean; data: any[] }>(
        `/sidak/rule-versions/${versionId}/indicators`,
      );
  ...
  const handleDeleteIndicator = async (indicatorId: string) => {
    try {
      await deleteApi(
        `/sidak/rule-versions/${expandedVersionId}/indicators/${indicatorId}`,
      );
  ...
  const handleAddIndicator = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await postApi(
        `/sidak/rule-versions/${addIndicatorVersionId}/indicators`,
        {
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${styles[status] || ""}`}
      >
        <span
          className={`w-1.5 h-1.5 rounded-full ${status === "draft" ? "bg-amber-500" : status === "published" ? "bg-emerald-500" : "bg-gray-400"}`}
        />
            ...
            <tbody className="divide-y">
              {(weights ?? []).map((w) => (
                <tr key={w.service_type} className="hover:bg-gray-50">
                  ...
                  <td
                    className={`p-3 text-center font-medium ${Math.abs(w.critical_weight + w.non_critical_weight - 1) < 0.01 ? "text-green-600" : "text-red-600"}`}
                  >
      ...
      {activeTab === "versions" && (
        <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        {(v as any).indicator_count !== undefined && (
                          <span className="text-[11px] text-gray-400">
                            · {(v as any).indicator_count}{" "}
                            {(v as any).indicator_count === 1
                              ? "indicator"
                              : "indicators"}
                          </span>
                        )}
                      ...
                      <div className="flex gap-4 text-xs text-gray-500">
                        <span>
                          Mode: <strong>{v.scoring_mode}</strong>
                        </span>
                        <span>
                          Critical: <strong>{v.critical_weight}</strong>
                        </span>
                        <span>
                          Non-Critical: <strong>{v.non_critical_weight}</strong>
                        </span>
                      </div>
                      {v.change_reason && (
                      ...
                      <div className="flex gap-4 text-[10px] text-gray-400">
                        <span>
                          ...
                          {v.created_by_user
                            ? ` by ${v.created_by_user.full_name}`
                            : ""}
                        </span>
                        {v.published_at && (
                          <span>
                            ...
                            {v.published_by_user
                              ? ` by ${v.published_by_user.full_name}`
                              : ""}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 ml-4 shrink-0">
                      {v.status === "draft" && (
                        <>
                          ...
                          <button
                            onClick={() =>
                              setActionTarget({ id: v.id, action: "publish" })
                            }
                      ...
                      {v.status === "published" && (
                        <button
                          onClick={() =>
                            setActionTarget({ id: v.id, action: "supersede" })
                          }
                  ...
                  {expandedVersionId === v.id && (
                    <div className="mt-4 border-t pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        {v.status === "draft" && (
                          <button
                            onClick={() => setAddIndicatorVersionId(v.id)}
                            ...
                            {compareIndicatorsLoading ? (
                              ...
                              (() => {
                                const draftNames = new Set(
                                  versionIndicators.map((i: any) => i.name),
                                );
                                const pubNames = new Set(
                                  compareIndicators.map((i: any) => i.name),
                                );
                                const added = versionIndicators.filter(
                                  (i: any) => !pubNames.has(i.name),
                                );
                                const removed = compareIndicators.filter(
                                  (i: any) => !draftNames.has(i.name),
                                );
                                const weightDiff =
                                  v.critical_weight !==
                                    compareVersion.critical_weight ||
                                  v.non_critical_weight !==
                                    compareVersion.non_critical_weight;
                                const modeDiff =
                                  v.scoring_mode !==
                                  compareVersion.scoring_mode;
                                return (
                                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 space-y-1">
                                    ...
                                    <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-[11px] text-blue-700">
                                      ...
                                      {added.length === 0 &&
                                        removed.length === 0 && (
                                          <span>No indicator changes</span>
                                        )}
                                      {weightDiff && (
                                        <span>Weights changed</span>
                                      )}
                                      {modeDiff && (
                                        <span>
                                          Mode: {compareVersion.scoring_mode} →{" "}
                                          {v.scoring_mode}
                                        </span>
                                      )}
                                    </div>
                      ...
                      {versionIndicatorsLoading ? (
                        ...
                        <p className="text-xs text-gray-400 italic py-2">
                          No indicators assigned to this version.
                        </p>
                      ) : (
                          ...
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="bg-gray-50 text-left">
                                ...
                                <th className="p-2 font-medium text-gray-500 text-right">
                                  Bobot
                                </th>
                                <th className="p-2 font-medium text-gray-500 text-center">
                                  NA
                                </th>
                                <th className="p-2 font-medium text-gray-500 text-right">
                                  Threshold
                            ...
                            <tbody className="divide-y">
                              {versionIndicators.map(
                                (ind: any, idx: number) => {
                                  const isNew =
                                    v.status === "draft" &&
                                    compareVersion &&
                                    !compareIndicators.some(
                                      (ci: any) => ci.name === ind.name,
                                    );
                                  return (
                                    <tr
                                      key={ind.id}
                                      className={`hover:bg-gray-50 ${isNew ? "bg-green-50" : ""}`}
                                    >
                                      ...
                                      <td className="p-2">
                                        <span
                                          className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold ${ind.category === "critical" ? "bg-red-100 text-red-700" : ind.category === "non_critical" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-500"}`}
                                        >
                                          {ind.category}
                                        </span>
                                      </td>
          ...
          {editingVersion && (
            ...
            >
              <form
                ...
                className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4 space-y-4"
              >
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-gray-900">
                    Edit Draft —{" "}
                    {SERVICE_LABELS[editingVersion.service_type] ||
                      editingVersion.service_type}{" "}
                    v{editingVersion.version_number}
                  </h3>
                  <button
                    type="button"
                    onClick={() => setEditingVersion(null)}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <label className="block">
                    ...
                    <select
                      name="scoring_mode"
                      defaultValue={editingVersion.scoring_mode}
                      className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-amber-500"
                    >
                      <option value="weighted">Weighted</option>
                      <option value="flat">Flat</option>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <label className="block">
                      ...
                      <input
                        ...
                        min="0"
                        max="1"
                        defaultValue={editingVersion.critical_weight}
                        className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-amber-500"
                      />
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        min="0"
                        max="1"
                        defaultValue={editingVersion.non_critical_weight}
                        className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-amber-500"
                      />
                    </label>
                  </div>
                  <label className="block col-span-2">
                    ...
                    <input
                      ...
                      defaultValue={editingVersion.change_reason || ""}
                      className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-amber-500"
                    />
                  </label>
                </div>
          ...
          {actionTarget &&
            (() => {
              ...
              const pv =
                ...
                versions?.find(
                  (o) =>
                    o.service_type === tv.service_type &&
                    o.status === "published",
                );
              const isExpanded = expandedVersionId === tv?.id;
              const indCount = isExpanded ? versionIndicators.length : null;
              const pubIndCount =
                isExpanded && pv && compareVersion?.id === pv.id
                  ? compareIndicators.length
                  : null;
              const indDiff =
                indCount !== null && pubIndCount !== null
                  ? indCount - pubIndCount
                  : null;
              ...
              return (
                ...
                >
                  <form
                    ...
                    onClick={(e) => e.stopPropagation()}
                    className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4 space-y-4"
                  >
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-gray-900">
                        ...
                        <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
                          <div className="flex items-center justify-between">
                            <span className="font-semibold">
                              {SERVICE_LABELS[tv.service_type] ||
                                tv.service_type}{" "}
                              v{tv.version_number}
                            </span>
                            <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-100 text-amber-700 border border-amber-200">
                              Draft → Published
                            </span>
                          </div>
                          <div className="flex gap-3 text-xs text-gray-500">
                            <span>
                              Mode: <strong>{tv.scoring_mode}</strong>
                            </span>
                            <span>
                              Critical: <strong>{tv.critical_weight}</strong>
                            </span>
                            <span>
                              Non-Critical:{" "}
                              <strong>{tv.non_critical_weight}</strong>
                            </span>
                          </div>
                        </div>
                        ...
                        {pv ? (
                          <div className="space-y-2">
                            <div className="bg-emerald-50 rounded-lg p-3 space-y-1 text-sm">
                              ...
                              <div className="flex gap-3 text-xs text-gray-500">
                                <span>
                                  Mode: <strong>{pv.scoring_mode}</strong>
                                </span>
                                <span>
                                  Critical:{" "}
                                  <strong>{pv.critical_weight}</strong>
                                </span>
                                <span>
                                  Non-Critical:{" "}
                                  <strong>{pv.non_critical_weight}</strong>
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 text-xs">
                              ...
                              {(tv.critical_weight !== pv.critical_weight ||
                                tv.non_critical_weight !==
                                  pv.non_critical_weight) && (
                              ...
                              {tv.scoring_mode !== pv.scoring_mode && (
                                <span className="px-2 py-1 rounded bg-amber-50 text-amber-700">
                                  Mode: {pv.scoring_mode} → {tv.scoring_mode}
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <p className="text-sm text-gray-600">
                            No previously published version — this will be the
                            first active rule set for this service type.
                          </p>
                        )}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-600">
                        This will deactivate this version. The version will no
                        longer be the active rule set.
                      </p>
                    )}
          ...
          {addIndicatorVersionId && (
            <div
              className="fixed inset-0 z-50 flex items-center justify-center bg-black/40"
              onClick={() => setAddIndicatorVersionId(null)}
            >
              <form
                  ...
                  >
                    ...
                    {(allIndicators ?? [])
                      .filter(
                        (i: any) =>
                          i.service_type ===
                          versions?.find((v) => v.id === addIndicatorVersionId)
                            ?.service_type,
                      )
                      .map((i: any) => (
