...
export async function getAccessScopeOptions(): Promise<AccessScopeOptions> {
  ...
  const VALID_SERVICES = ['call', 'chat', 'email', 'cso', 'pencatatan', 'bko', 'slik'];
  ...
  if (error) {
    return {
      teams: [],
      services: VALID_SERVICES.map((srv) => ({ value: srv, label: SERVICE_LABELS[srv] || srv })),
      agentsByTeam: {}
    };
  }
...
export async function getPendingLeaderRequests(): Promise<PendingLeaderRequest[]> {
  const { data, error } = await supabaseAdmin
...
export async function getApprovedLeaderRequests(): Promise<ApprovedLeaderAccess[]> {
...
export async function revokeLeaderRequest(
  ...
  const { error } = await supabaseAdmin
    .from('leader_access_requests')
    .update({ status: 'revoked', reviewed_by: reviewerId, review_note: note || null })
    .eq('id', requestId)
    .eq('status', 'approved');
