...
export function normalizeTelefunCoachingRecommendations(
  value: unknown,
): TelefunCoachingRecommendation[] {
...
export function normalizePdktEvaluation(
  value: unknown,
): PdktEvaluationResult | null {
  const full = pdktEvaluationAiOutputSchema.safeParse(value);
  const raw = object(value);
  const { scoreBreakdown: _scoreBreakdown, ...legacyInput } = raw;
  const legacy = pdktEvaluationAiOutputSchema
    .omit({ scoreBreakdown: true })
    .safeParse(legacyInput);
...
export function normalizeTelefunAssessmentWithHold(
  value: unknown,
  ...
  const hold = evaluateTelefunHoldAssessment(
    normalizeTelefunHoldMetrics(metrics.hold),
  );
...
function sourceError(table: string, error: { message?: string }): Error {
  return new Error(
    `Monitoring source ${table} failed: ${error.message || "database error"}`,
  );
}
...
async function readCoaching(
  ...
  for (const ids of batches(sessionIds)) {
    const result = await client
      ...
      .select("id, session_id, recommendations, generated_at")
      .in("session_id", ids);
    if (result.error)
      throw sourceError("telefun_coaching_summary", result.error);
    const candidates = ((result.data || []) as SourceRow[]).filter((summary) =>
      string(summary.session_id),
    );
    candidates.sort((left, right) => {
      const generated = string(right.generated_at).localeCompare(
...
export function normalizePdktMetadata(
  configValue: unknown,
  emails: EmailMessage[],
): ConsumerMetadata {
  ...
  const firstEmail = object(
    emails.find((email) => object(email).type === "received") || emails[0],
  );
  const recipient =
    nullableString(recipientContext.primaryRecipientAddress) ||
    emailValue(firstEmail, ["to", "recipient"]);
  const contact =
    emailValue(firstEmail, ["from", "sender", "senderEmail"]) ||
    nullableString(identity.email);
  return {
    consumer_name:
      nullableString(identity.name) || nullableString(identity.bodyName),
...
function userFields(
  row: SourceRow,
  profiles: Map<string, SourceRow>,
): Pick<UnifiedHistoryEntry, "user_email" | "user_role"> {
  const profile = row.user_id ? profiles.get(row.user_id) : undefined;
  return {
    user_email: nullableString(profile?.email) || undefined,
    user_role: nullableString(profile?.role) || undefined,
  };
}

function telefunAssessment(
  value: unknown,
  sessionMetrics: unknown,
): MonitoringTelefunAssessment | undefined {
...
export async function getMonitoringHistory(): Promise<UnifiedHistoryEntry[]> {
  ...
  const [ketikRows, pdktRows, telefunRows, legacyRows] = await Promise.all([
    readComplete(
      admin,
      "ketik_history",
      "id, user_id, date, scenario_title, consumer_name, consumer_phone, consumer_city, messages, simulation_duration, final_score, empathy_score, probing_score, resolution_score, typo_score, compliance_score, review_status, simulation_subject_type, simulation_subject_peserta_id, simulation_subject_name, simulation_subject_batch_name, simulation_subject_team",
    ),
    readComplete(
      admin,
      "pdkt_history",
      "id, user_id, timestamp, config, emails, evaluation, evaluation_status, evaluation_error, time_taken, created_at, simulation_subject_type, simulation_subject_peserta_id, simulation_subject_name, simulation_subject_batch_name, simulation_subject_team",
    ),
    readComplete(
      admin,
      "telefun_history",
      "id, user_id, created_at, scenario_title, consumer_name, consumer_phone, consumer_city, consumer_gender, duration_seconds, recording_path, score, messages, voice_assessment, session_metrics, ai_summary, strengths, weaknesses, coaching_focus, persona_config, simulation_subject_type, simulation_subject_peserta_id, simulation_subject_name, simulation_subject_batch_name, simulation_subject_team",
    ),
    readComplete(
      admin,
      "results",
      "id, user_id, module, score, details, history, created_at",
      (query) => query.eq("module", "telefun"),
    ),
  ]);

  const userIds = [
    ...new Set(
      [...ketikRows, ...pdktRows, ...telefunRows, ...legacyRows]
        .map((row) => row.user_id)
        .filter((id): id is string => Boolean(id)),
    ),
  ];
  const profiles = await readProfiles(admin, userIds);
  const coaching = await readCoaching(
    admin,
    telefunRows.map((row) => row.id).filter((id): id is string => Boolean(id)),
  );
  ...
  for (const row of ketikRows) {
    ...
    entries.push({
      ...
      module: "ketik",
      scenario_title: string(row.scenario_title, "Simulasi Chat"),
      created_at: string(row.date),
      duration_seconds: number(row.simulation_duration),
      score: typeof row.final_score === "number" ? row.final_score : null,
      history: messages,
      ...userFields(row, profiles),
      ...
      review_status: status(row.review_status),
      scores: {
        final:
          typeof row.final_score === "number" ? row.final_score : undefined,
        empathy:
          typeof row.empathy_score === "number" ? row.empathy_score : undefined,
        probing:
          typeof row.probing_score === "number" ? row.probing_score : undefined,
        resolution:
          typeof row.resolution_score === "number"
      ...
      ketik_session: {
        ...
        messages,
        simulation_duration:
          typeof row.simulation_duration === "number"
  ...
  for (const row of pdktRows) {
    ...
    entries.push({
      ...
      scenario_title: string(
        object(array(config?.scenarios)[0]).title,
        "Simulasi Email",
      ),
      created_at: string(row.timestamp || row.created_at),
      duration_seconds: number(row.time_taken),
      score: typeof evaluation?.score === "number" ? evaluation.score : null,
      history: emails,
      ...userFields(row, profiles),
      ...metadata,
      review_status: status(row.evaluation_status),
      pdkt_evaluation: summary,
      simulationSubject: mapSimulationSubjectRowToSnapshot(row),
      pdkt_session: {
  ...
  for (const row of telefunRows) {
    ...
    canonicalIds.add(id);
    const assessment = telefunAssessment(
    ...
    entries.push({
      ...
      user_id: string(row.user_id),
      module: "telefun",
      scenario_title: string(row.scenario_title, "Simulasi Telepon"),
      created_at: string(row.created_at),
      duration_seconds: number(row.duration_seconds),
      score,
      history: parseTelefunTranscript(row.messages).length
        ? parseTelefunTranscript(row.messages)
        : safeRecordingPath(row.recording_path),
      ...userFields(row, profiles),
      ...metadata,
      review_status: score !== null ? "completed" : "not_started",
      simulationSubject: mapSimulationSubjectRowToSnapshot(row),
      telefun_assessment: assessment || undefined,
      telefun_coaching: {
        recommendations: normalizeTelefunCoachingRecommendations(
          summary?.recommendations,
        ),
        generated_at: summary ? nullableString(summary.generated_at) : null,
      },
    });
  }
  ...
  for (const row of legacyRows) {
    ...
    entries.push({
      ...
      module: "telefun",
      scenario_title: string(
        details.scenario || details.scenario_title,
        "Simulasi Telepon",
      ),
      created_at: string(row.created_at),
      duration_seconds: number(details.duration),
      score: typeof row.score === "number" ? row.score : null,
      history:
        safeRecordingPath(details.recordingUrl) ||
        parseTelefunTranscript(row.history),
      ...userFields(row, profiles),
      simulationSubject: mapSimulationSubjectRowToSnapshot(null),
      review_status:
