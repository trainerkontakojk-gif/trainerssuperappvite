...
ai.post('/generate', zValidator('json', generateSchema), async (c) => {
  ...
  const { modelId, provider } = resolveModelProvider(body.model || 'gemini-3.1-flash-lite');
  ...
  if (!response.success) {
    return c.json({ success: false, error: { code: 'AI_ERROR', message: response.error || 'AI tidak tersedia.' } }, 502);
  }
...
ai.get('/usage', async (c) => {
  ...
  if (error) return c.json({ success: false, error: { code: 'DB_ERROR', message: error.message } }, 500);
...
ai.get('/monitoring/aggregation', async (c) => {
  ...
  for (const log of logs) {
    if (!userAgg[log.user_id]) {
      userAgg[log.user_id] = { user_id: log.user_id, total_calls: 0, total_input_tokens: 0, total_output_tokens: 0, total_tokens: 0, total_cost_idr: 0, models: {} };
    }
    ...
    if (!agg.models[key]) agg.models[key] = { model_id: log.model_id, module: log.module, calls: 0, input_tokens: 0, output_tokens: 0, total_tokens: 0, cost_idr: 0 };
    const m = agg.models[key];
    m.calls += 1; m.input_tokens += log.input_tokens || 0; m.output_tokens += log.output_tokens || 0;
    m.total_tokens += log.total_tokens || 0; m.cost_idr += log.estimated_cost_idr || 0;
  }
...
ai.get('/monitoring/pricing', async (c) => {
  ...
  const result = AI_MODELS.map(m => ({
    ...
    input_price_usd_per_million: pricingMap.get(m.id)?.input_price_usd_per_million ?? 0,
    output_price_usd_per_million: pricingMap.get(m.id)?.output_price_usd_per_million ?? 0,
  }));

  for (const p of dbPricing) {
    if (!result.some(r => r.model_id === p.model_id)) {
      result.push({ model_id: p.model_id, model_name: p.model_id, provider: 'gemini' as const, input_price_usd_per_million: p.input_price_usd_per_million, output_price_usd_per_million: p.output_price_usd_per_million });
    }
  }
...
ai.put('/monitoring/pricing', requireRole('admin', 'trainer'), zValidator('json', pricingUpsertSchema), async (c) => {
  const body = c.req.valid('json');
...
ai.get('/monitoring/billing', async (c) => {
  ...
  if (error) return c.json({ success: false, error: { code: 'DB_ERROR', message: error.message } }, 500);
  return c.json({ success: true, data: { usd_to_idr_rate: data?.usd_to_idr_rate ?? 15000 } });
});

const billingUpdateSchema = z.object({ usd_to_idr_rate: z.number().min(1).max(100000) });

ai.post('/monitoring/billing', requireRole('admin', 'trainer'), zValidator('json', billingUpdateSchema), async (c) => {
  const body = c.req.valid('json');

  const admin = createAdminClient();
  const { error } = await admin.from('ai_billing_settings').insert({ usd_to_idr_rate: body.usd_to_idr_rate });
  if (error) return c.json({ success: false, error: { code: 'DB_ERROR', message: error.message } }, 500);
  return c.json({ success: true, data: null });
});
