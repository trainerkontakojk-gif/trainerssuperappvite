...
export function isServiceType(
  ...
  return (
    typeof value === "string" &&
    VALID_SERVICE_TYPES.includes(value as ServiceType)
  );
}
...
export function computeEffectiveService(
  ...
  return (
    serviceOverride ??
    (agentTim ? resolveServiceTypeFromTeam(agentTim) : null) ??
    fallbackService ??
    "call"
  );
}
...
export function calculateQAScoreFromTemuan(
  ...
  const buildDetail = (cat: "critical" | "non_critical") => {
    const targetInds =
      weight.scoring_mode === "no_category"
        ? indicators
    ...
    return targetInds.map((ind) => {
      ...
      const avgNilai =
        matchingTemuan.length > 0
          ? matchingTemuan.reduce((a, b) => a + b.nilai, 0) /
            matchingTemuan.length
          : 3;
      return {
        indicatorId: ind.id,
        name: ind.name,
  ...
  return {
    ...
    nonCriticalDetail:
      weight.scoring_mode === "no_category" ? [] : buildDetail("non_critical"),
    criticalDetail:
      weight.scoring_mode === "no_category" ? [] : buildDetail("critical"),
