...
function drawSectionTitle(page: any, font: any, text: string, y: number): number {
...
function drawBodyText(page: any, font: any, text: string, y: number): number {
  ...
  for (const line of lines) {
    if (y < 60) return -1;
    page.drawText(line, { x: MARGIN, y, size: FONT_SIZE_BODY, font, color: DARK });
    y -= LINE_HEIGHT;
  }
  return y - 4;
}

function drawBulletList(page: any, font: any, items: string[], y: number): number {
  for (const item of items) {
    if (y < 60) return -1;
    const lines = wrapText(item, font, FONT_SIZE_BODY, CONTENT_WIDTH - 12);
    page.drawText('•', { x: MARGIN, y, size: FONT_SIZE_BODY, font, color: DARK });
    for (const line of lines) {
      if (y < 60) return -1;
      page.drawText(line, { x: MARGIN + 12, y, size: FONT_SIZE_BODY, font, color: DARK });
      y -= LINE_HEIGHT;
    }
    y -= 2;
  }
...
function drawTable(page: any, font: any, headers: string[], rows: any[][], y: number): number {
  const colCount = headers.length;
  const colWidth = CONTENT_WIDTH / colCount;

  const headerY = y;
  page.drawRectangle({
    x: MARGIN, y: headerY - 16, width: CONTENT_WIDTH, height: 16,
    color: TABLE_HEADER,
  });

  let xPos = MARGIN + 4;
  for (let i = 0; i < headers.length; i++) {
    page.drawText(headers[i], { x: xPos, y: headerY - 12, size: FONT_SIZE_SMALL, font, color: WHITE });
    xPos += colWidth;
  }
  y = headerY - 20;
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      for (let j = 0; j < lines.length; j++) {
        page.drawText(lines[j], { x: xPos, y: rowY - j * 11 - 2, size: FONT_SIZE_SMALL, font, color: DARK });
      }
      xPos += colWidth;
    }
    y -= maxHeight + 2;
...
export async function buildAiReportPdf(params: ReportParams): Promise<Uint8Array> {
  ...
  page.drawText(params.title, {
    x: MARGIN, y, size: FONT_SIZE_TITLE, font: boldFont, color: BLUE,
  });
  y -= 24;

  // Subtitle
  page.drawText('Laporan Analisis Kualitas QA', {
    x: MARGIN, y, size: FONT_SIZE_H2, font, color: GRAY,
  });
  y -= 18;
  ...
  for (const line of metaLines) {
    page.drawText(line, { x: MARGIN, y, size: FONT_SIZE_SMALL, font, color: GRAY });
    y -= 12;
  }
  y -= 10;
  ...
  const cardY = y;
  page.drawRectangle({
    x: MARGIN, y: cardY - 36, width: cardWidth, height: 36, color: LIGHT,
  });
  page.drawText('Total Temuan', {
    x: MARGIN + 8, y: cardY - 24, size: FONT_SIZE_SMALL, font, color: GRAY,
  });
  page.drawText(String(params.totalFindings), {
    x: MARGIN + 8, y: cardY - 14, size: FONT_SIZE_H1, font: boldFont, color: BLUE,
  });

  page.drawRectangle({
    x: MARGIN + cardWidth + 12, y: cardY - 36, width: cardWidth, height: 36, color: LIGHT,
  });
  page.drawText('Total Baris Data', {
    x: MARGIN + cardWidth + 20, y: cardY - 24, size: FONT_SIZE_SMALL, font, color: GRAY,
  });
  page.drawText(String(params.totalRows), {
    x: MARGIN + cardWidth + 20, y: cardY - 14, size: FONT_SIZE_H1, font: boldFont, color: BLUE,
  });
  y = cardY - 48;
  ...
  if (y < 60) { page = doc.addPage([PAGE_WIDTH, PAGE_HEIGHT]); y = PAGE_HEIGHT - MARGIN; }
  ...
  if (params.chartData) {
    ...
    if (cd.donutData && cd.donutData.total > 0) {
      if (y < 80) { page = doc.addPage([PAGE_WIDTH, PAGE_HEIGHT]); y = PAGE_HEIGHT - MARGIN; }
      y -= 8;
      y = drawSectionTitle(page, boldFont, 'Distribusi Temuan', y);
      if (y < 60) { page = doc.addPage([PAGE_WIDTH, PAGE_HEIGHT]); y = PAGE_HEIGHT - MARGIN; }
      const donutRows = [
        ['Critical', String(cd.donutData.critical), `${((cd.donutData.critical / cd.donutData.total) * 100).toFixed(1)}%`],
        ['Non-Critical', String(cd.donutData.nonCritical), `${((cd.donutData.nonCritical / cd.donutData.total) * 100).toFixed(1)}%`],
        ['Total', String(cd.donutData.total), '100%'],
      ];
      y = drawTable(page, font, ['Kategori', 'Jumlah', 'Persentase'], donutRows, y);
    }
    ...
    if (cd.paretoData && cd.paretoData.length > 0) {
      ...
      if (y < 60) { page = doc.addPage([PAGE_WIDTH, PAGE_HEIGHT]); y = PAGE_HEIGHT - MARGIN; }
      const paretoRows = cd.paretoData.slice(0, 10).map(p => [p.name, String(p.count), String(p.cumulative)]);
      y = drawTable(page, font, ['Parameter', 'Jumlah', 'Kumulatif'], paretoRows, y);
    }
  ...
  lastPage.drawText(`Laporan digenerate otomatis melalui Trainers SuperApp — ${now}`, {
    x: MARGIN, y: 30, size: FONT_SIZE_SMALL, font, color: GRAY,
  });
