...
describe('KETIK Service - Settings', () => {
  ...
  describe('getSettings', () => {
    ...
    it('returns stored settings when row exists', async () => {
      ...
      const settings = await ketikService.getSettings('user1');
      expect(settings.selectedModel).toBe('openrouter/openai/gpt-4o-mini');
  ...
  describe('saveSettings', () => {
    it('saves settings for own user', async () => {
      ...
      await expect(ketikService.saveSettings('user1', DEFAULT_KETIK_SETTINGS)).resolves.toBeUndefined();
...
describe('KETIK Service - History', () => {
          ...
          data: [
            {
              ...
              consumer_name: 'Budi',
              messages: [{ id: 'm1', sender: 'agent', text: 'hello', timestamp: '2025-01-15T10:00:00Z' }],
  ...
  describe('deleteSession', () => {
    it('deletes only for own user', async () => {
      ...
      await expect(ketikService.deleteSession('sess1', 'user1')).resolves.toBeUndefined();
    });
  });
