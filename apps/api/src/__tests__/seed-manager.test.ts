...
const productionDbUrlArb = fc.oneof(
  ...
  fc.string({ minLength: 1, maxLength: 20 }).map(
    (prefix) => `postgresql://user:pass@db.${prefix}.supabase.co:5432/postgres`,
  ),
  ...
  fc.string({ minLength: 1, maxLength: 20 }).map(
    (prefix) => `postgresql://user:pass@${prefix}-prod-db.example.com:5432/mydb`,
  ),
  // URLs containing 'production' in the host
  fc.string({ minLength: 1, maxLength: 20 }).map(
    (prefix) => `postgresql://user:pass@${prefix}.production.example.com:5432/mydb`,
  ),
  // Direct pattern inclusion with arbitrary surrounding text
  fc.constantFrom(...PRODUCTION_HOST_PATTERNS).chain((pattern) =>
    fc.tuple(
...
class IdempotentTable<T extends { id: string }> {
   ...
   */
  insertOnConflictDoNothing(record: T): { success: boolean; skipped: boolean; error?: string } {
...
const seedDataSetArb = fc.record({
  ...
  profilerPeserta: fc.array(profilerPesertaRecordArb, { minLength: 1, maxLength: 20 }),
...
const conflictStrategyArb = fc.constantFrom<ConflictStrategy>('doNothing', 'doUpdate');
  ...
  it('double-application with ON CONFLICT DO NOTHING produces no errors and stable row counts', () => {
    fc.assert(
      fc.property(
        ...
        (seedData) => {
          // Create fresh tables
          const profilesTable = new IdempotentTable<(typeof seedData.profiles)[0]>();
          const pesertaTable = new IdempotentTable<(typeof seedData.profilerPeserta)[0]>();
          const periodsTable = new IdempotentTable<(typeof seedData.qaPeriods)[0]>();
          const aiLogsTable = new IdempotentTable<(typeof seedData.aiUsageLogs)[0]>();
    ...
    fc.assert(
      fc.property(
        ...
        (seedData, strategy, applicationCount) => {
          ...
          const applyFn = strategy === 'doNothing' ? applySeedDoNothing : applySeedDoUpdate;
