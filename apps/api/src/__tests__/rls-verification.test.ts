...
type AccessPattern = 'owner-only' | 'role-based' | 'service-role-insert' | 'mixed';
type Operation = 'SELECT' | 'INSERT' | 'UPDATE' | 'DELETE';
type Role = 'anon' | 'authenticated' | 'admin' | 'trainer' | 'leader' | 'user' | 'service_role';
...
function evaluateRLSPolicy(
  ...
  switch (config.accessPattern) {
    case 'owner-only': {
      ...
      if (role === 'authenticated' || role === 'admin' || role === 'trainer' || role === 'leader' || role === 'user') {
    ...
    case 'role-based': {
      ...
      if (policy.allowedRoles.includes('authenticated') &&
          (role === 'authenticated' || role === 'admin' || role === 'trainer' || role === 'leader' || role === 'user')) {
        return { allowed: true, policyName: policy.policyName };
      }
      if (policy.allowedRoles.includes(role)) {
        return { allowed: true, policyName: policy.policyName };
...
describe('RLS Verification Test Suite', () => {
  ...
  describe('Owner-Only Access Pattern', () => {
    ...
    it('negative case: anon cannot access owner-only tables', () => {
      for (const config of OWNER_ONLY_TABLES) {
        for (const policy of config.policies) {
          const result = evaluateRLSPolicy(config, policy.operation, 'anon', false);
    ...
    it('negative case: non-owner authenticated user cannot access others data (except admin on profiles)', () => {
      ...
      for (const config of tablesWithoutAdminOverride) {
        for (const policy of config.policies) {
          ...
          const result = evaluateRLSPolicy(config, policy.operation, 'user', false);
          expect(result.allowed).toBe(false);
        }
      }
    });
  ...
  describe('Role-Based Access Pattern (admin/trainer)', () => {
    ...
    it('positive case: admin/trainer can write to role-based tables', () => {
      for (const config of ROLE_BASED_TABLES) {
        const writePolicy = config.policies.find((p) => p.operation === 'INSERT');
        if (writePolicy) {
          const adminResult = evaluateRLSPolicy(config, 'INSERT', 'admin', false);
          const trainerResult = evaluateRLSPolicy(config, 'INSERT', 'trainer', false);
    ...
    it('positive case: authenticated users can SELECT from role-based tables with read_all policy', () => {
      ...
      const readAllTables = ROLE_BASED_TABLES.filter((t) =>
        t.policies.some((p) => p.operation === 'SELECT' && p.allowedRoles.includes('authenticated')),
      );
      for (const config of readAllTables) {
        const result = evaluateRLSPolicy(config, 'SELECT', 'authenticated', false);
        expect(result.allowed).toBe(true);
      }
    });
    ...
    it('negative case: regular user/leader cannot write to role-based tables', () => {
      for (const config of ROLE_BASED_TABLES) {
        ...
        if (writePolicy) {
          ...
          const leaderResult = evaluateRLSPolicy(config, 'INSERT', 'leader', false);
  ...
  describe('Service-Role-Only INSERT Pattern', () => {
    it('ai_usage_logs: only service_role can INSERT', () => {
      const config = SERVICE_ROLE_INSERT_TABLES.find((t) => t.table === 'ai_usage_logs')!;
      const insertPolicy = config.policies.find((p) => p.operation === 'INSERT')!;

      // Service role can insert
      const serviceResult = evaluateRLSPolicy(config, 'INSERT', 'service_role', false);
      expect(serviceResult.allowed).toBe(true);

      // Authenticated users cannot insert directly
      const authResult = evaluateRLSPolicy(config, 'INSERT', 'authenticated', false);
    ...
    it('ai_usage_logs: owner can SELECT their own logs', () => {
      ...
      // Owner can select
      const ownerResult = evaluateRLSPolicy(config, 'SELECT', 'authenticated', true);
  ...
  describe('Mixed Access Pattern', () => {
    it('leader_access_requests: leader can view own, admin/trainer can manage all', () => {
      const config = MIXED_ACCESS_TABLES.find((t) => t.table === 'leader_access_requests')!;
      ...
      // Trainer can select
      const trainerResult = evaluateRLSPolicy(config, 'SELECT', 'trainer', false);
      expect(trainerResult.allowed).toBe(true);
    ...
    it('ai_pricing_settings: authenticated can read, anon denied', () => {
      const config = MIXED_ACCESS_TABLES.find((t) => t.table === 'ai_pricing_settings')!;

      const authResult = evaluateRLSPolicy(config, 'SELECT', 'authenticated', false);
    ...
    it('ai_billing_settings: authenticated can read, anon denied', () => {
      const config = MIXED_ACCESS_TABLES.find((t) => t.table === 'ai_billing_settings')!;
  ...
  describe('Comprehensive RLS Failure Report (Requirement 7.3)', () => {
    ...
    it('failure report includes required fields: table, policy, operation, role, issue', () => {
      ...
      expect(mockFailure.issue).toMatch(/incorrectly_granted|incorrectly_denied/);
    });
  });
