  ...
  createAdminClient: () => ({
    from: vi.fn((table: string) =>
      table === "telefun_coaching_summary"
        ? {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({ maybeSingle: coachingSummary })),
            })),
          }
        : { select },
    ),
...
describe("Telefun monitoring review transcript", () => {
  ...
  it("selects messages, returns transcript entries, and signs recording URL for trainer", async () => {
    single.mockResolvedValue({
      data: {
        ...
        user_id: "trainer-1",
        recording_path:
          "trainer-1/00000000-0000-0000-0000-000000000001/full_call.webm",
        agent_recording_path:
          "trainer-1/00000000-0000-0000-0000-000000000001/agent_only.webm",
    ...
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("agent_recording_path"),
    );
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("consumer_phone"),
    );
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("consumer_city"),
    );
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("simulation_subject_type"),
    );
    expect(body.data.simulationSubject).toEqual({
      type: "participant",
    ...
    expect(body.data.recording_url).toBe(
      "https://storage.example/signed-full-call.webm",
    );
    expect(body.data.agent_recording_path).toBe(
      "trainer-1/00000000-0000-0000-0000-000000000001/agent_only.webm",
    );
    ...
    expect(body.data.persona_config).toEqual({
      consumerType: "Marah & Emosional",
    });
  ...
  it("returns canonical coaching recommendations joined by session_id", async () => {
    coachingSummary.mockResolvedValue({
      data: {
        recommendations: [{ text: "Perjelas penutupan", priority: 2 }],
    ...
    single.mockResolvedValue({
      data: {
        ...
        consumer_city: "Bandung",
        consumer_gender: "female",
        persona_config: { consumerType: "Rendah" },
      },
      error: null,
    });
    const response = await buildApp().request(
    ...
    expect(body.data.consumer_city).toBe("Bandung");
    expect(body.data.consumer_gender).toBe("female");
    expect(body.data.coaching_recommendations).toEqual([
      { text: "Perjelas penutupan", priority: 2 },
    ]);
  ...
  it("drops malformed coaching recommendations and returns an explicit empty result", async () => {
    coachingSummary.mockResolvedValue({
      data: {
        recommendations: [
          { text: "Valid", priority: 5 },
          { text: "", priority: 2 },
          null,
          { text: "Bad priority", priority: Infinity },
        ],
        generated_at: null,
      },
      error: null,
  ...
  it("returns PDKT subject, actor, batch/team, and status projections", async () => {
    single.mockResolvedValue({
      data: {
        user_id: "trainer-1",
    ...
    const response = await buildApp().request(
      "/monitoring/history/pdkt/00000000-0000-0000-0000-000000000001/review",
    );
    const body = await response.json();
  ...
  it("returns ketik monitoring details with simulation duration and resolution score", async () => {
    ...
    expect(response.status).toBe(200);
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("simulation_duration"),
    );
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("resolution_score"),
    );
    expect(select).toHaveBeenCalledWith(
      expect.stringContaining("consumer_phone"),
    );
    ...
    expect(body.data.scores).toMatchObject({
      final: 0,
      empathy: 0,
      probing: 0,
      resolution: 0,
  ...
  it("rejects external recording URLs without leaking them", async () => {
    single.mockResolvedValue({
      data: {
        score: 8,
        recording_path: "https://attacker.example/call.webm",
        agent_recording_path: null,
        scenario_title: "Tagihan",
        duration_seconds: 60,
        voice_assessment: null,
  ...
  it("returns 500 when coaching summary read fails", async () => {
    ...
    coachingSummary.mockResolvedValue({
      data: null,
      error: { message: "coaching unavailable" },
    });
    const response = await buildApp().request(
      "/monitoring/history/telefun/00000000-0000-0000-0000-000000000001/review",
    );
  ...
  it("does not sign monitoring recording URLs for leader", async () => {
    ...
    expect(createSignedUrl).not.toHaveBeenCalled();
    expect(body.data.recording_url).toBeNull();
    expect(body.data.recording_path).toBe(
      "trainer-1/00000000-0000-0000-0000-000000000001/full_call.webm",
    );
  });
});
