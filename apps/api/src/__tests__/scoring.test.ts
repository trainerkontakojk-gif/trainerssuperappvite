...
describe('calculateQAScoreFromTemuan', () => {
  const indicators: QAIndicator[] = [
    makeIndicator({ id: 'ind-1', name: 'Salam', category: 'non_critical', bobot: 2 }),
    makeIndicator({ id: 'ind-2', name: 'Verifikasi', category: 'critical', bobot: 3 }),
    makeIndicator({ id: 'ind-3', name: 'Empati', category: 'non_critical', bobot: 1 }),
  ];
  ...
  it('handles no_category scoring mode', () => {
    ...
    const temuan = [
      { indicator_id: 'ind-1', nilai: 3, no_tiket: 'TKT-001' },
    ];
