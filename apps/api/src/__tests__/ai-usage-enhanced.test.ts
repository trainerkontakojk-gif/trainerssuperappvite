...
const statusArb = fc.constantFrom<RequestStatus>('success', 'failed', 'timeout');
...
describe('Property 9: AI Usage Status and Error Recording', () => {
  ...
  describe('Failure/timeout case properties', () => {
    const failureStatusArb = fc.constantFrom<RequestStatus>('failed', 'timeout');
    ...
    it('error_message preserves content up to 1000 chars (prefix match)', () => {
      fc.assert(
        fc.property(
          failureStatusArb,
          tokenUsageArb,
          fc.string({ minLength: 1, maxLength: 3000 }).filter((s) => s.trim() !== ''),
          (status, tokens, errorMessage) => {
            const result = computeAiUsageFields({
              status,
              errorMessage,
